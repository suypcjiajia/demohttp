...
int TMysqlDAL::Query( const string& sSql )
{
    ...
    //tLog( LOG_ERROR,"miOperationTimes:%d",this->miOperationTimes);
    try {
        int iMyRet = mysql_query( &this->mMysql, sSql.c_str() );
        if ( 0 != iMyRet) {
           // int errno = mysql_errno(&this->mMysql);
            tLog( LOG_ERROR, "%s,errorno:%d,erorstr:%s",sSql.c_str(),mysql_errno(&this->mMysql),mysql_error( &this->mMysql ) );
