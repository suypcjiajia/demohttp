...
void ProcessCall2(EvhtpSvr * pEvhtp){
    if(pEvhtp->GetUrl() == "/user/add"){
        ...
        userDataBLL.AddData(pEvhtp);
    }else if(pEvhtp->GetUrl() == "/user/update"){
        UserDataBLL userDataBLL;
        userDataBLL.Update(pEvhtp);
    }else if(pEvhtp->GetUrl() == "/user/info"){
        UserDataBLL userDataBLL;
        userDataBLL.GetUserInfo(pEvhtp);
    }
    else{
        RetJsonMsg::SetRetMsg(-12,pEvhtp->GetJsonResponse());
    }
}
int main(int argc, const char* argv[]) {
    ...
    //初始化config
    if ( 0 != MyConfig::GetInstance()->Init()) {
        return -1;
    }
    ...
    MysqlConnect::GetInstance()->Init("demo");//设置数据库名
